...
void Weapon_RocketLauncher_Fire (edict_t *ent)
{
	...
	VectorSet(offset, 88, 8, ent->viewheight-8);
	P_ProjectSource (ent->client, ent->s.origin, offset, forward, right, start);
...
void Machinegun_Fire (edict_t *ent)
{
	...
	*/
	...
	for (i=1 ; i<3 ; i++)
	{
		ent->client->kick_origin[i] = crandom() * 0.05;
		ent->client->kick_angles[i] = crandom() * 0.17;
	}
	ent->client->kick_origin[0] = crandom() * 0.05;
	ent->client->kick_angles[0] = ent->client->machinegun_shots * -0.5;
...
void Chaingun_Fire (edict_t *ent)
{
	...
	for (i=0 ; i<3 ; i++)
	{
		ent->client->kick_origin[i] = crandom() * 4;
...
void Weapon_Chaingun (edict_t *ent)
{
	...
	static int	fire_frames[]	= {5,  7,  9, 10, 12, 13, 15, 16,  18, 19, 21, 0};
