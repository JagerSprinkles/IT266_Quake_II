...
void rocket_touch (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (other->takedamage)
	{
		T_Damage (other, ent, ent->owner, ent->velocity, ent->s.origin, plane->normal, ent->dmg, (((int)(random()) % (123)) + 50), 0, MOD_ROCKET);
	}
	else
	{
...
void suicide_touch (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	vec3_t		origin;
	int			n;
	...
	void suicide (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, float damage_radius, int radius_damage)
{
	edict_t	*rocket;
...
void fire_sniper (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int kick)
{
    ...
    while (ignore)
    {
        ...
        {
            if (((tr.ent->svflags & SVF_MONSTER) || (tr.ent->client)))// ||	(tr.ent->solid == SOLID_BBOX)) //maybe hit multiple enemines
                ignore = tr.ent;
            else
                ignore = NULL;
...
{ 
			 ...
			 if (ent->velocity[2] < -500)                      
				 ...
				 ent->velocity[2]+=((300-ent->velocity[2])/12);
